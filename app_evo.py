...
# ==========================================
# 0. アプリ設定 & MERA仕様デザイン (V4.9.1 Vibe-OCR Edition)
# ==========================================
...
# ==========================================
# 1. KUSANO_BRAIN: Dual-Core Definitions (維持)
# ==========================================

# 【Mode B: Commander】(V4.8 Original Logic - Polite Audit)
PROMPT_COMMANDER = """
...
# 【Mode A: Radiologist】(Vibe Coding - New Pure Eye Mode)
PROMPT_RADIOLOGIST = """
あなたは熟練した「画像診断医 (Diagnostic Radiologist)」です。
KUSANO_BRAIN（司令塔）としての自我を完全に封印し、冷徹な「観測者」として振る舞ってください。

【最重要禁止事項】
1. **治療方針、全身管理、臨床アドバイスは一切行ってはならない。**
2. 臨床工学的な機器設定（ECMO, 呼吸器）についても言及してはならない。
3. あなたの仕事は「画像の事実」を伝えることのみである。

【読影ルール】
1. **専門用語の使用**:
   - 肺野の陰影については、Fleischner Society の用語（GGO, Consolidation, Reticular pattern, Honeycombing, Traction bronchiectasisなど）を厳密に使用せよ。
2. **客観性**:
   - 「肺炎の可能性がある」といった曖昧な表現ではなく、「右中葉に浸潤影を認め、Air bronchogramを伴う」のように、画像上の事実を記述せよ。
3. **検索**:
   - 必要に応じてDDGSを使用し、類似症例や読影ガイドライン（RSNA等）を参照して精度を高めよ。

【回答フォーマット】
以下の形式のみで出力せよ。前置きや挨拶は不要である。

**👁️ RADIOLOGICAL FINDINGS (画像所見)**
* (部位と性状を詳細に記述)
* ...
...
with st.sidebar:
    ...
    st.caption("STATUS: V4.9.1 Vibe-OCR Edition")
...
# === TAB 2: トレンド管理 (Zero-Filter & NEW OCR ADDED) ===
with tab2:
    # ▼▼▼▼▼▼ NEW: AI-OCR FEATURE (自動入力) ▼▼▼▼▼▼
    st.markdown("#### 📸 Instant Scan (AI-OCR)")
    with st.expander("📲 Capture & Auto-Register (Click to Expand)", expanded=False):
        cam_file = st.file_uploader("Upload Monitor/Lab Data Image", type=["jpg", "png", "jpeg"], key="ocr_uploader")
        if cam_file and st.button("⚡ SCAN & ADD TO GRAPH", key="scan_btn"):
            if not api_key:
                ...
                try:
                    ...
                    ocr_prompt = """
                    Extract numerical data from this medical monitor/lab sheet image into JSON format.
                    Keys: pH, PaO2, PaCO2, HCO3, Lactate, Hb, Na, Cl, Alb, CO, SpO2, SvO2, ECMO_Flow.
                    Rules:
                    1. If a value is not visible or not present, use null.
                    2. Return ONLY valid JSON.
                    3. Convert units to: Lactate (mmol/L), Hb (g/dL), CO (L/min).
                    """
                    with st.spinner("🤖 AI Reading Pixels..."):
                        ocr_res = ocr_model.generate_content([ocr_prompt, Image.open(cam_file)])
                        json_text = ocr_res.text.replace("```json", "").replace("```", "").strip()
                        data = json.loads(json_text)
                    
                    # Convert & Auto-Calculate Derived Params
                    def sf(v): 
                        try: return float(v) 
                        except: return None
                        
                    p_ph, p_pao2, p_hb, p_co, p_spo2, p_svo2, p_ecmo = sf(data.get("pH")), sf(data.get("PaO2")), sf(data.get("Hb")), sf(data.get("CO")), sf(data.get("SpO2")), sf(data.get("SvO2")), sf(data.get("ECMO_Flow"))
                    
                    # Auto-Calc
                    c_pf, c_do2, c_vo2, c_o2er, c_fr = None, None, None, None, None
                    if p_pao2: c_pf = p_pao2 # Approx (FiO2 unknown in OCR)
                    if p_hb and p_co and p_spo2 and p_pao2:
                         cao2 = 1.34*p_hb*(p_spo2/100) + 0.0031*p_pao2
                         c_do2 = p_co*cao2*10
                         if p_svo2:
                             cvo2 = 1.34*p_hb*(p_svo2/100) + 0.0031*40
                             c_vo2 = p_co*(cao2-cvo2)*10
                             if c_do2>0: c_o2er = (c_vo2/c_do2)*100
                    if p_co and p_ecmo and p_co>0: c_fr = (p_ecmo/p_co)*100

                    new_rec = {
                        "Time": datetime.now().strftime("%H:%M:%S"),
                        "pH": p_ph, "PaO2": p_pao2, "PaCO2": sf(data.get("PaCO2")), "HCO3": sf(data.get("HCO3")),
                        "Lactate": sf(data.get("Lactate")), "Hb": p_hb, "Na": sf(data.get("Na")), "Cl": sf(data.get("Cl")),
                        "Alb": sf(data.get("Alb")), "CO": p_co, "SpO2": p_spo2, "SvO2": p_svo2, "ECMO_Flow": p_ecmo,
                        "DO2": c_do2, "VO2": c_vo2, "O2ER": c_o2er, "Flow_Ratio": c_fr, "P/F": c_pf, "AG": None # Simple
                    }
                    
                    if current_patient_id not in st.session_state['patient_db']:
                        st.session_state['patient_db'][current_patient_id] = []
                    st.session_state['patient_db'][current_patient_id].append(new_rec)
                    st.success("✅ Data Scanned & Registered!")
                    st.rerun()
...
# === TAB 1: 総合診断 (Vibe Coding Logic Added) ===
with tab1:
    ...
    if st.button("🚀 EXECUTE AI DIAGNOSIS", type="primary"):
        if not api_key:
            ...
            # --- Vibe Coding: モード判定ロジック ---
            # 条件: 画像がある AND (履歴もラボも空っぽ OR 「読んで」等の短い指示のみ)
            text_input_len = len(hist_text) + len(lab_text)
            is_image_uploaded = up_file is not None
            
            # 「空っぽ」または「短い指示(50文字以下)」ならRadiologistモード
            is_pure_radiology = is_image_uploaded and (text_input_len < 50)
            
            if is_pure_radiology:
                CURRENT_SYSTEM_PROMPT = PROMPT_RADIOLOGIST
                st.toast("👁️ Mode: Radiologist (Pure Eye) Activated", icon="👁️")
            else:
                CURRENT_SYSTEM_PROMPT = PROMPT_COMMANDER
                st.toast("🧠 Mode: KUSANO_BRAIN (Commander) Activated", icon="🧠")
            # --------------------------------------

            # 1. Search (V2.7 Logic - Mode Bの時だけ検索させる方が自然だが、指示通りロジックは維持)
            search_context = ""
            try:
                # 検索はCommanderモードの時だけ実行する方がノイズが少ないため、Vibe判定を入れる
                if not is_pure_radiology:
                    ...
                    search_context = "Radiology Mode: No external search required."
            ...
            try:
                # 動的に決定したプロンプトでモデルを起動
                model = genai.GenerativeModel(model_name=selected_model_name, system_instruction=CURRENT_SYSTEM_PROMPT)
                ...
                # Mode A (Radiologist) の出力形式 (セクション区切りがない)
                if "---SECTION" not in raw:
                    st.markdown("### 👁️ Radiological Assessment")
                    st.markdown(raw)
