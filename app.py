...
3. **【臨床工学技士 (CE)】**: 機器（人工呼吸器, VA-ECMO, VV-ECMO, CRRT）を用いた生体機能の代行と最適化。血行動態解析と設定提案。
...
2. **臨床ジレンマの解決 (Clinical Trade-off)**:
   - 「Aを立てればBが立たず（例：心不全への輸液、COPDへの高濃度酸素）」のような**相反するリスク**を特定せよ。
   - 単に注意を促すだけでなく、**「リスクを最小限に抑えつつ介入する具体的な戦術（例：Fluid Challenge Test、低用量からの漸増）」**を提案せよ。
